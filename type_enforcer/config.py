...
class Config:
    ...
    exclude_paths: List[str] = field(
        default_factory=lambda: [".git", "__pycache__", "venv", "env", ".env"]
    )
    ...
    def _load_types_from_file(self):
        ...
        for file_path in files_to_load:
            ...
            if path.suffix == ".json":
                ...
                raise ValueError(
                    f"Неподдерживаемый формат файла с типами: {path.suffix}"
                )
    ...
    def get_import_for_type(
        self, type_name: str, current_file_path: Union[str, Path]
    ) -> Optional[str]:
        """
            ...
            try:
                ...
                if up_levels > 0:
                    ...
                    if down_parts:
                        ...
                        return (
                            f"from {dots}{module_path}.{module_name} import {type_name}"
                        )
                    else:
